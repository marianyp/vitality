package dev.mariany.vitality.util;

public final class VitalityConstants {
    public static final int HEALTHY_EATING_WINDOW = 10;
    public static final float MIN_BUFF_RATIO = 0.7F;

    public static final float FAST_FALL_THRESHOLD = -0.6F;
    public static final float FAST_FALL_DAMP = 0.2F;
    public static final int WALL_SLIDE_DELAY = 16;
    public static final float SLIDE_SPEED = -0.1F;
    public static final float WALL_JUMP_HEIGHT = 0.5F;
    public static final int MAX_WALL_JUMPS = 4;

    public static final float MAX_SOFT_LAND_HEIGHT = 16.5F;
    public static final float SOFT_LAND_DAMAGE_MULTIPLIER = 0.6F;

    public static final int ROLL_DURATION = 11;
}
