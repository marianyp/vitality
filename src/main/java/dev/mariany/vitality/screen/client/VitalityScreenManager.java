...
public class VitalityScreenManager {
    ...
    public static void update(TextRenderer textRenderer, DrawContext context, int mouseX, int mouseY) {
        ...
        if (isHovered(mouseX, mouseY)) {
            context.drawTooltip(
                    textRenderer, Text.translatable("gui.inventory.vitality.diet", tooltipText), mouseX,
                    mouseY
            );
        }
    }
    ...
    public static void drawDiet(DrawContext context) {
        ...
        if (score > 0) {
            ...
            context.drawGuiTexture(
                    ...
                    DIET,
                    SPRITE_WIDTH,
                    FILLED_SPRITE_HEIGHT,
                    0,
                    FILLED_SPRITE_HEIGHT - revealedHeight,
                    x + xOffset,
                    (y + yOffset + 1) + (FILLED_SPRITE_HEIGHT - revealedHeight),
                    SPRITE_WIDTH,
                    revealedHeight
            );
        }
